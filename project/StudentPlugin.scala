...
object StudentPlugin extends AutoPlugin {
  ...
  val packageSubmissionZipSettings = Seq(
    packageSubmissionZip := {
      ...
      IO.zip(Seq(sources -> "sources.zip", binaries -> "binaries.jar"),
             submission)
      submission
    },
    ...
    mappings in (Compile, packageBin) := {
      val relativePaths =
        (unmanagedResources in Compile).value.flatMap(
          Path.relativeTo((unmanagedResourceDirectories in Compile).value)(_))
  ...
  val styleCheckSetting = styleCheck := {
    val (_, sourceFiles, info, assignmentName) = ((compile in Compile).value,
